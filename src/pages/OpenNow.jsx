...
function OpenNow() {
	...
	const OpenNow = data.filter((item) => item.open_now_text === 'Open Now');
  ...
  return (
	<div className='w-[100%] px-[5rem] pb-[5rem]'>
		<h1 className='mb-[30px] text-[30px]'>Open Restaurant</h1>
		<div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-[50px]'>			

			{OpenNow.slice(0, visibleItems).map((item, index) => (
				<Card key={index} {...item}/>
			))}
		</div>
