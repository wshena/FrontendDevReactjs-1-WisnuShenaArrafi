...
function Asian() {
	...
	const filter = "Asian";
	const Asian = filteredData.filter((item) => item?.cuisine[0]?.name === filter)
  ...
  return (
	<div className='w-[100%] px-[5rem]'>
		<h1 className='mb-[30px] text-[30px]'>Asian Restaurant</h1>
		<div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-[50px]'>

			{Asian.slice(0, visibleItems).map((item, index) => (
				<Card key={index} {...item}/>
			))}
		</div>
