...
function Cheap() {
	...
	const filter = "$";
	const cheap = filteredData.filter((item) => item?.price_level === filter)
  ...
  return (
	<div className='w-[100%] px-[5rem]'>
		<h1 className='mb-[30px] text-[30px]'>Cheap Restaurant</h1>
		<div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-[50px]'>

			{cheap.slice(0, visibleItems).map((item, index) => (
				<Card key={index} {...item}/>
			))}
		</div>
